...
class BCAgent:
	...
	def act(self, obs, goal, step): # not using the goal here
		...
		if self.use_encoder:
			# TODO for pixel space
			pass
		
		dist_action = self.actor(obs, stddev) # we are not using stddev as of now because there is no exploration it's a supervised policy
	...
	def update(self, expert_replay_iter, step):
		...
		dist = self.actor(obs, stddev) # mean action
		log_prob = dist.log_prob(action).sum(axis = -1, keepdim = True) # sum the loss along both dimensions
		...
		# set the actor's optimizer to zero
